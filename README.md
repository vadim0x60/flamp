...
`flamp` runs an Amp prompt on a disposable Fly Machine, commits the result, and pushes it back to your GitHub branch. Launch your agent, close your laptop, get a push notification to your phoen when it's done.
...
`FLAMP_NTFY_TOPIC` | unset | Optional ntfy topic for Amp's final output.
`FLAMP_NTFY_SERVER` | `https://ntfy.sh` | ntfy server base URL.
