...
- The [Amp CLI](https://ampcode.com/) installed locally. `flamp` uses it to generate `Dockerfile.amp` (the project's base image + dependency-install steps, plus flamp's scaffolding) when one does not already exist.
...
On the first run, flamp generates `Dockerfile.amp` — the complete build recipe (base image + dependency installs, authored by Amp from your repo, plus flamp's scaffolding) — and persists it. On later runs that file is built **verbatim**; flamp never overwrites it. It carries a `# Generated by flamp` header so you can tell it's safe to delete and regenerate; a `Dockerfile.amp` you write by hand is also built verbatim and left untouched.
...
- For full control over the image, commit your own `Dockerfile.amp` (without the `# Generated by flamp` header). When present, *any* `Dockerfile.amp` — generated or hand-written — is built verbatim and flamp will not overwrite it. A hand-written one must copy the build context into `/tmp/home/work`, install dependencies, and install the Amp CLI itself. The build context no longer contains `.git`; the runner initializes git at runtime, so you don't need to bake it in (if you do, the runner reuses it).
